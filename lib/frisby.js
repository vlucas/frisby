...
var _withPath = function(path, jsonBody, callback) {
  ...
  // ONE item in array should match
    ...
    if (match) {
      // ?{\d} syntax means exactly matches
      numRequired = parseInt(match[1]);
      failed = numRequired !== itemCount - errorCount;
    }
    else {
      // old behavior of "at least one matches"
      failed = itemCount === errorCount;
    }

    // If all errors, test fails
    if(failed === true && !self.current.isNot) {
      if(errorLast) {
        throw errorLast;
      } else {
        throw new Error("Expected " + numRequired + " object(s) in path '" + path + "' to match provided JSON values");
      }
    }
...
function Frisby(msg) {
  ...
  this.current = {
    ...
    //The time in millis to wait before retrying. May also be a function(retry_count) { return wait_in_millis; }
...
// HTTP header expect helper (this just verifies that the header exists as a string)
Frisby.prototype.expectHeaderExistsAsString = function(header) {
  var self = this;
  var header = (header+"").toLowerCase();
  this.current.expects.push(function() {
    if(typeof self.current.response.headers[header] !== "undefined") {
      expect(self.current.response.headers[header]).toBeType(String);
    } else {
      throw new Error("Header '" + header + "' not present in HTTP response");
    }
  });
  ...
  this.current.expects.push(function() {
    ...
    if(failed === true && !self.current.isNot) {
        if(errorLast) {
          ...
          throw new Error("Expected " + numRequired + " object(s) in path '" + path + "' to match provided JSON types");
        }
      }
...
Frisby.prototype.inspectBody = function() {
  this.afterRaw(function(err, res, body) {
    console.log(body);
  });
  return this;
};
...
Frisby.prototype.after = function(cb) {
  ...
  this.current.after.push(function() {
    var body = self.current.response.body;
    // parse string unless it's empty which would normally be a head request
    // if you want plain text use afterRaw
    if (typeof body === 'string' && body !== '') {
        try {
            body = _jsonParse(body);
        } catch(err) {
            // not an error really, it's probably just a po file instead of json
        }
    }
    cb.call(this, self.current.response.error, self.currentRequestFinished.res, body);
  });
  return this;
};

// Callback function to run after test is completed
Frisby.prototype.afterRaw = function(cb) {
  var self = this;
  this.current.after.push(function() {
    var body = bignumJSON.stringify(self.current.response.body);
    cb.call(this, self.current.response.error, self.currentRequestFinished.res, body);
  });
  return this;
};
...
Frisby.prototype.setResponseJSON = function(json) {
  ...
  this.current.response.body = bignumJSON.stringify(json);
  return json;
};
      ...
      for (var x = 0; x < retries + 1; x++) {
        runs(function() {
          ...
          if (this.n > 0) {
            ...
            retry_backoff = self.current.retry_backoff;
            if (typeof self.current.retry_backoff === "function") {
              retry_backoff = self.current.retry_backoff(this.n);
            }
            waits(retry_backoff);
          }
...
function _jsonParse(body) {
  json = "";
  try {
    json = (typeof body === "object") ? body : bignumJSON.parse(body);
  } catch(e) {
