...
var globalSetup = function(opts) {
  ...
  _frisbyGlobalSetup.pathSplitChar = _frisbyGlobalSetup.pathSplitChar || defaultPathSplitChar;
...
var _withPath = function(path, jsonBody, callback) {
  var self = this
    , type = false
    // Mainly for tests
    , pathSplitChar = self.pathSplitChar || defaultPathSplitChar;

  try {
    // Use given path to check deep objects
    if(path) {
      var pathParts = path.split(pathSplitChar);
      _.each(pathParts, function(segment) {
...
function Frisby(msg) {
  ...
  this.pathSplitChar = _gs.pathSplitChar;
...
Frisby.prototype.expectJSONTypes = function(/* [tree], jsonTest */) {
  ...
  this.current.expects.push(function() {
    ...
    try {
      // Use given path to check deep objects
      if(path) {
        _.each(path.split(self.pathSplitChar), function(segment) {
          jt = _toType(jsonBody);
