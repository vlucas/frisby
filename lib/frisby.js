...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
  ...
  this.current.it = function () {
    ...
    runner(outgoing, function(err, res, body) {
      ...
      //console.log("\nResult == \n");
      //console.log(self.currentRequestFinished);
...
Frisby.prototype.expectJSONLength = function(jsonTest) {
  var self     = this,
      args     = Array.prototype.slice.call(arguments),
      path     = typeof args[0] === 'string' && args.shift(),
      jsonTest = typeof args[0] === 'number' && args.shift(),
      type     = null;
...
function _jsonContains(jsonBody, jsonTest) {
  if(typeof jsonTest !== "object") {
    throw {
      name: "TypeError",
      message: "Expected valid JavaScript object to be given, got " + typeof expected
    }
  }
