...
var _withPath = function(path, jsonBody, callback) {
  ...
  if("*" === type || "&" === type) {
    _.each(jsonBody, function(json) {
      callback.call(self, json, self.current.isNot);
    });
  ...
  // ONE item in array should match
    ...
    for(var i = 0; i < itemCount; i++) {
      try {
        var result = callback.call(self, jsonBody, self.current.isNot);
      } catch (e) {
        errorCount++;
        errorLast = e;
      }
  ...
  // Normal matcher
    ...
    return callback.call(self, jsonBody, self.current.isNot);
  }
};
...
Frisby.prototype.expectJSONTypes = function(/* [tree], jsonTest */) {
  ...
  this.current.expects.push(function() {
    ...
    // ONE item in array should match
      ...
      for(var i = 0; i < itemCount; i++) {
        try {
          var result = _jsonContainsTypes.call(self, jsonBody[i], jsonTest, self.current.isNot);
        } catch (e) {
          errorCount++;
          errorLast = e;
        }
...
Frisby.prototype.expectJSONSchema = function(path, jsonSchema) {
  ...
  this.current.expects.push(function() {
    var jsonBody = _jsonParse(self.current.response.body);
    // With path syntax
    _withPath.call(self, path, jsonBody, function() {
...
jasmine.Matchers.prototype.toContainJsonTypes = function(expected, isNot) {
  ...
  // Way of allowing custom failure message - by thowing expections in utility function
  try {
    return _jsonContainsTypes.call(this, this.actual, expected);
  } catch(e) {
    // Fail test if there is a match failure and it is not an inverse test (for non-match)
    if(!this.isNot && !isNot) {
      this.spec.fail(e);
