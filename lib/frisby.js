...
//
Frisby.prototype.timeout = function(_t) {
  var t = _t || null;
  if(null === t) {
...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
  ...
  if(_.isUndefined(outgoing.headers['content-type']) && !_.isUndefined(outgoing.body)) {
...
Frisby.prototype.expectHeader = function(_header, content) {
  var self = this;
...
// Helper to check parse HTTP response body as JSON and check key types
Frisby.prototype.expectJSON = function() {
  var self     = this,
      args     = Array.prototype.slice.call(arguments),
      path     = typeof args[0] === 'string' && args.shift(),
      jsonTest = typeof args[0] === 'object' && args.shift(),
...
Frisby.prototype.expectJSONSchema = function() {
  var self       = this,
      args       = Array.prototype.slice.call(arguments),
      path       = typeof args[0] === 'string' && args.shift(),
...
Frisby.prototype.expectJSONLength = function() {
  var self           = this,
      args           = Array.prototype.slice.call(arguments),
  ...
  for(var key in jsonTest) {
    ...
    if(jsonBody) {
      ...
      if(kt === "object") {
        ...
        if(!test || arguments[2]) {
          throw new Error("Expected " + _toType(jsonBody[key]) + " '" + jsonBody[key] + "' to " + (arguments[2]?'NOT':'') + " match " + _toType(jsonTest[key]) + " '" + jsonTest[key] + "' on key '" + key + "'");
        }
      }
